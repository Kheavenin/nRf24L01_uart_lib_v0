...
void clearInterruptFlags(nrfStruct_t *nrfStruct)
{
...
uint8_t getInterruptFlags(nrfStruct_t *nrfStruct) {
	uint8_t tmp = readReg(nrfStruct, STATUS);
	tmp = 4 >> (tmp & 0x70);
	if (tmp & 0x01)
		nrfStruct->statusStruct.maxRetr = 1;
	if (tmp & 0x02)
		nrfStruct->statusStruct.dataSendIrq = 1;
	if (tmp & 0x04)
		nrfStruct->statusStruct.dataReadIrq = 1;
	return tmp;
}
